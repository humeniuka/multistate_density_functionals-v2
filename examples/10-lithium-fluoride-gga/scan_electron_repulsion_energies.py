...
with the multi-state generalized gradient approximation (GGA) for the exchange functional

    Wᵢⱼ ≈ Jᵢⱼ[D] - Kᵢⱼ[D] + Cᵢⱼ[D] + SIC δᵢⱼ
        = 1/2 ∑ₖ ∫∫' Dᵢₖ(r) Dₖⱼ(r')/|r-r'|
          - 2¹ᐟ³ Cₓ ∑ᵅ ∫ Dᵅ(r)⁴ᐟ³ F(X²(r)ᵅ) dr
...
on the geometry nor on the electronic state.
    ...
    SIC = - 2 x ( J[ρ₁ₛᵅ] - K[ρ₁ₛᵅ(r)] + C[ρ₁ₛᵅ] ) δᵢⱼ
...
if __name__ == "__main__":
    ...
    for bond_length in bond_lengths:
        ...
        SIE = CoreSelfInteractionCorrection(
            mol,
            exchange_functional_class=GGABecke88ExchangeLikeFunctional,
        ...
        # K[D]
        exchange_functional = GGABecke88ExchangeLikeFunctional(mol)
        # C[D]
        correlation_functional = LDACorrelationLikeFunctional(mol)
        # kinetic energy
        # T[D] = T_{LLP}[D]
        kinetic_functional_LLP = GGALeeLeeParr91KineticFunctional(mol)
        ...
        K_GGA = exchange_functional(msmd)
        print("calculating C[D] ...")
        C_LDA = correlation_functional(msmd)
        W_approximate = J_Hartree - K_GGA + C_LDA + SIC * numpy.eye(nstate)
        ...
        print("calculating T_LLP[D] (Lee,Lee & Parr 1991 GGA)...")
        T_approximate = kinetic_functional_LLP(msmd)
        ...
        scan_data['K_GGA'].append(K_GGA.tolist())
